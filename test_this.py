...
plt.plot(range(180), norm_from_pi, marker=".")
plt.show()
...
def ee(i,d): # i-th standard basis vector in R^d
    return sparse.coo_matrix(([1], ([0], [i])),  (1, d))

def Delta(i,j,d): # 0-1 d-by-d matrix with 1 only at the (i,j)-th entry
    return sparse.coo_matrix(([1], ([i], [j])),  (d, d))
...
def neighbor_list(M):
    ...
    boundary = np.argwhere(d<2).flatten()
...
#C = np.vstack(c3 + [c4])
C_sparse = c3 + [c4]
print('checkpoint:', sparse.issparse(As[2]))

# x = cvx.Variable(2*m+n)
# obj = cvx.sum_squares(x[-n:]-pi)
# consQ = [cvx.quad_form(x, As[i]) + bs[i] @ x == 0 for i in range(g)] # Quadratic constrains
# #consL = [C @ x == np.ones((n+1,1))] # Linear constraints
# consL = [c @ x == 1 for c in C_sparse] # Linear constraints
# cons = consQ + consL + [x >= np.zeros(2*m+n)] #np.array([1]*m+[0]*m+[1]*n)/N
# prob = cvx.Problem(cvx.Minimize(obj), cons)

# qcqp = QCQP(prob)
# qcqp.suggest() # qcqp.suggest(SDR, solver=cvx.MOSEK)
...
# print(np.array([c @ x1 for c in C_sparse]) - np.ones(n+1))

print('time:', time.time()-t0)

#ff = open('Modified_M.txt','w+')
#ff.write(str((M,pi,neighbors)))
#ff.write(str((x1,x1[-n:],pi,obj.value)))
#ff.close()
...
def W_from_x(v,neighbs,n):
    IJs = []
    for i,a in enumerate(neighbs):
        for j in a:
            IJs.append([i,j])
    W = np.zeros((n,n))
    for k,a in enumerate(IJs):
        W[a[0],a[1]] = v[k]
    W = W/np.sum(W,0)
    #print(np.linalg.norm(np.matmul(W,pi) - pi))
    return W

def W_sol_convex(M01,pi,N):
	B,c,LB,neighbors,lengths = opt_setup(M,pi,N)
	x_old = lsq_linear(B, c, bounds=(LB, np.inf),
	method='trf', tol=1e-10, lsq_solver=None, lsmr_tol=None,
	max_iter=None, verbose=0).x
	return W_from_x(x_old,neighbors,B.shape[0]//3)
    
def performance(W,q_init,pi,T,p):
	q = q_init #pi2
	errs = [np.linalg.norm(np.array(q) - pi,p)]
	for i in range(T):
		q = np.matmul(W,q)
	errs.append(np.linalg.norm(q - pi),p)
	return errs


W_sol = W_sol_convex(adj_mat,pi,N)

#print(W_sol)

I1 = np.argwhere(W_sol != 0)
print('I1',I1)

print(np.linalg.norm(np.matmul(W_sol,pi)-pi,1))

W_real0 = total_trains_along_each_edge.to_numpy()
W_real = W_real0/np.sum(W_real0,0)

#print(W_real)

I2 = np.argwhere(W_real != 0)
print('I2',I2)

print(np.linalg.norm(np.matmul(W_real,pi)-pi,1))
...
# 
# x_old = lsq_linear(B, c, bounds=(LB, np.inf), # solution
#                 method='trf', tol=1e-10, lsq_solver=None, lsmr_tol=None, 
#                 max_iter=None, verbose=0).x
...
#     W_sol[i,j] = weight(i,j,x_old,neighbors,lengths)
...
# print(W_sol)
# 
# print(np.sum(W_sol,0))
# 
# print(x_old)
# 
# with open("old_formulation_W2.txt", "w") as f:
#   f.write(str(x_old))

# ff = open('old_formulation_W.txt','w+')
# ff.write(str((W_sol,pi,neighbors)))
# #ff.write(str((x1,x1[-n:],pi,obj.value)))
# ff.close()
